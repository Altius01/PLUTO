 ...
 *********************************************************************** */
{
  ...
  if (prank == 0)
    RuntimeSetup(&runtime, &cmd_line, input_file);
#ifdef PARALLEL
  ...
  while (!last_step) {
    ...
    if (cmd_line.jet != -1)
      SetJetDomain(&data, cmd_line.jet, runtime.log_freq, grd);
    {
      int i, j, k;
    ...
    if (cmd_line.jet != -1)
      UnsetJetDomain(&data, cmd_line.jet, grd);
 ...
 *********************************************************************** */
{
  ...
  if (g_stepNumber % runtime->log_freq == 0) {
    ...
    print("%s [dt   (particles) =       %10.4e]\n", str,
          1.0 / Dts->invDt_particles);
#if PARTICLES == PARTICLES_CR
    print("%s [1/omL(particles) =       %10.4e]\n", str,
          1.0 / Dts->omega_particles);
#endif
#endif
  }
#endif
...
#if (PARABOLIC_FLUX & SUPER_TIME_STEPPING) || (PARABOLIC_FLUX & RK_LEGENDRE)
 ...
 *********************************************************************** */
{
  ...
  for (n = 0; n < MAX_OUTPUT_TYPES; n++) {
    ...
    if (output->dt > 0.0) {
      check_dt =
          (int)((g_time + g_dt) / output->dt) - (int)(g_time / output->dt);
