...
#define MAX_OP 9 /* Maximum number of diffusion operators */
...
#if HAVE_ENERGY
#if (AMBIPOLAR_DIFFUSION == EXPLICIT) || (RESISTIVITY == EXPLICIT) ||          \
    (THERMAL_CONDUCTION == EXPLICIT) || (VISCOSITY == EXPLICIT)
...
/*!
 * \param [in]  d             Pointer to the PLUTO data structure.
 * \param [out] dU            3D array containing the conservative right hand
 *sides \param [in]  domBox        A pointer to an RBox structure defining the
 *zones of the domain to be updated \param [in]  aflux         A 2D pointer to
 *store fluxes (needed by Chombo) \param [in]  timeStepping  an integer
 *specifying the time stepping method (= EXPLICIT / STS / RKL) \param [in]  dt
 *the time step \param [in]  grid          pointer to Grid structure
 *
 ...
 *********************************************************************** */
{
  ...
  for (nv = 0; nv < MAX_OP; nv++) {
    if (C_dtp[nv] != NULL)
      TOT_LOOP(k, j, i) C_dtp[nv][k][j][i] = 0.0;
  }
  ...
  if (include[TC_OP]) {
    TOT_LOOP(k, j, i)
    d->Tc[k][j][i] = d->Vc[PRS][k][j][i] / d->Vc[RHO][k][j][i];
  }
#endif
        ...
        if (include[RES_OP]) {
          ...
          if (g_intStage == 1) {
            ...
            IBOX_LOOP(domBox, i) {
              ...
              C_dtp[RES_OP + 0][k][j][i] +=
                  0.5 * (dcoeff_res[0][i - 1] + dcoeff_res[0][i]) * inv_dl2;
              C_dtp[RES_OP + 1][k][j][i] +=
                  0.5 * (dcoeff_res[1][i - 1] + dcoeff_res[1][i]) * inv_dl2;
              C_dtp[RES_OP + 2][k][j][i] +=
                  0.5 * (dcoeff_res[2][i - 1] + dcoeff_res[2][i]) * inv_dl2;
        ...
        if (include[RES_OP]) {
          ...
          if (g_intStage == 1) {
            ...
            JBOX_LOOP(domBox, j) {
              ...
              C_dtp[RES_OP + 0][k][j][i] +=
                  0.5 * (dcoeff_res[0][j - 1] + dcoeff_res[0][j]) * inv_dl2;
              C_dtp[RES_OP + 1][k][j][i] +=
                  0.5 * (dcoeff_res[1][j - 1] + dcoeff_res[1][j]) * inv_dl2;
              C_dtp[RES_OP + 2][k][j][i] +=
                  0.5 * (dcoeff_res[2][j - 1] + dcoeff_res[2][j]) * inv_dl2;
        ...
        if (include[RES_OP]) {
          ...
          if (g_intStage == 1) {
            ...
            KBOX_LOOP(domBox, k) {
              ...
              C_dtp[RES_OP + 0][k][j][i] +=
                  0.5 * (dcoeff_res[0][k - 1] + dcoeff_res[0][k]) * inv_dl2;
              C_dtp[RES_OP + 1][k][j][i] +=
                  0.5 * (dcoeff_res[1][k - 1] + dcoeff_res[1][k]) * inv_dl2;
              C_dtp[RES_OP + 2][k][j][i] +=
                  0.5 * (dcoeff_res[2][k - 1] + dcoeff_res[2][k]) * inv_dl2;
...
#if THERMAL_CONDUCTION
    if (include[TC_OP])
      scrh = MAX(scrh, C_dtp[TC_OP][k][j][i]);
#endif

#if VISCOSITY
    if (include[VISC_OP])
      scrh = MAX(scrh, C_dtp[VISC_OP][k][j][i]);
#endif
