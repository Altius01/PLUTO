...
#define DOMI_LOOP(k, j, i)                                                     \
  KDOM_LOOP(k) JDOM_LOOP(j) for (i = IBEG - INCLUDE_IDIR; i <= IEND; i++)
#define DOMJ_LOOP(k, j, i)                                                     \
  KDOM_LOOP(k)                                                                 \
  for (j = JBEG - INCLUDE_JDIR; j <= JEND; j++)                                \
  IDOM_LOOP(i)
#define DOMK_LOOP(k, j, i)                                                     \
  for (k = KBEG - INCLUDE_KDIR; k <= KEND; k++)                                \
...
#define BOX_LOOP(B, k, j, i)                                                   \
  for ((B)->dk = ((k = (B)->kbeg) <= (B)->kend ? 1 : -1);                      \
       k != (B)->kend + (B)->dk; k += (B)->dk)                                 \
    for ((B)->dj = ((j = (B)->jbeg) <= (B)->jend ? 1 : -1);                    \
         j != (B)->jend + (B)->dj; j += (B)->dj)                               \
      for ((B)->di = ((i = (B)->ibeg) <= (B)->iend ? 1 : -1);                  \
           i != (B)->iend + (B)->di; i += (B)->di)
...
#define BOX_TRANSVERSE_LOOP(box, k, j, i)                                      \
  ...
  for (*((box)->b) = *(box)->bbeg; *((box)->b) <= *(box)->bend;                \
       *((box)->b) += 1)                                                       \
    for (*((box)->t) = *(box)->tbeg; *((box)->t) <= *(box)->tend;              \
         *((box)->t) += 1)
...
#define FOR_EACH(nv, list)                                                     \
  for ((list)->i = 0, nv = (list)->indx[0]; (list)->i < (list)->nvar;          \
       nv = (list)->indx[++((list)->i)])
 ...
 */
#define MINMOD_LIMITER(a, b)                                                   \
  ((a) * (b) > 0.0 ? (fabs(a) < fabs(b) ? (a) : (b)) : 0.0)
#define VANLEER_LIMITER(a, b)                                                  \
  ((a) * (b) > 0.0 ? 2.0 * (a) * (b) / ((a) + (b)) : 0.0)
#define MC_LIMITER(a, b)                                                       \
  (MINMOD_LIMITER(0.5 * ((a) + (b)), 2.0 * MINMOD_LIMITER((a), (b))))
...
#define DIM_LOOP(d)                                                            \
  for ((d) = 0; (d) < DIMENSIONS; (d) += 2 * INCLUDE_IDIR - INCLUDE_JDIR)
#else
#define DIM_LOOP(d)                                                            \
  for ((d) = 0;                                                                \
...
#define AVERAGE_XZ(q, k, j, i)                                                 \
  (0.5 * (AVERAGE_X(q, k, j, i) + AVERAGE_X(q, k + 1, j, i)))
#define AVERAGE_YZ(q, k, j, i)                                                 \
  (0.5 * (AVERAGE_Y(q, k, j, i) + AVERAGE_Y(q, k + 1, j, i)))
#define AVERAGE_XYZ(q, k, j, i)                                                \
  (0.5 * (AVERAGE_XY(q, k, j, i) + AVERAGE_XY(q, k + 1, j, i)))
#else
