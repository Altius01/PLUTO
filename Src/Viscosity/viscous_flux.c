...
\tens{I} \f]
  ...
  \f[
     \frac{1}{r}\pd{(rv_r)}{r} \approx 2\frac{V_{r,i+1}|r_{i+1}| -
V_{r,i}|r_{i}|} {r_{i+1}|r_{i+1}| - r_i|r_{i}|} \f] The source terms are
computed using Eq. (64) of [Mig14], \f[ \left(\frac{T_{\phi\phi}}{r}\right)_i
\approx \frac{(T_{\phi\phi})_{i-\HALF} + (T_{\phi\phi})_{i+\HALF}}{2r_i}
  ...
  \f[
    ...
    \Pi = \nu_1\left[\begin{array}{ccc}
      \DS  2\pd{v_r}{r}
    & \DS   r\pd{}{r}\left(\frac{v_\theta}{r}\right) +
\frac{1}{r}\pd{v_r}{\theta} & \DS   \frac{1}{r\sin\theta}\pd{v_r}{\phi} +
r\pd{}{r}\left(\frac{v_\phi}{r}\right)
    \\ \noalign{\medskip}
      \DS  \Pi_{r\theta}
    & \DS  2\left(\frac{1}{r}\pd{v_\theta}{\theta} + \frac{v_r}{r}\right)
    & \DS
\frac{\sin\theta}{r}\pd{}{\theta}\left(\frac{v_\phi}{\sin\theta}\right)
...
#div := 2*(abs(r[i+1])*V[i+1] - abs(r[i])*V[i])/(r[i+1]*abs(r[i+1]) -
r[i]*abs(r[i])); div := 3*(r[i+1]^2*V[i+1] - r[i]^2*V[i])/(r[i+1]^3 - r[i]^3);
 ...
 ************************************************************************* */
{
  ...
  if (Vphi_tot == NULL)
    Vphi_tot = ARRAY_3D(NX3_TOT, NX2_TOT, NX1_TOT, double);
  ...
  if (one_dVr == NULL) {
    one_dVr =
        ARRAY_1D(NX1_TOT, double); /* -- intercell (i) and (i+1) volume -- */
    one_dmu =
        ARRAY_1D(NX2_TOT, double); /* -- intercell (j) and (j+1) volume -- */
  ...
  if (g_dir == IDIR) {
    ...
    for (i = beg; i <= end; i++) {
      ...
      tau_xx[i] = tau_xy[i] = tau_xz[i] = tau_yx[i] = tau_yy[i] = tau_yz[i] =
          tau_zx[i] = tau_zy[i] = tau_zz[i] = 0.0;
      ...
      divV = DIM_EXPAND(
          (Vx[k][j][i + 1] * r[i + 1] - Vx[k][j][i] * fabs(r[i])) * one_dVr[i],
          +dyVy, +0.0);

      tau_xx[i] = 2.0 * nu1 * dxVx + (nu2 - (2.0 / 3.0) * nu1) * divV;
      tau_xy[i] = nu1 * (dyVx + dxVy);
      tau_xz[i] =
          nu1 * 0.5 * (r[i] + r[i + 1]) * inv_dx1 *
          ((1. / r[i + 1]) * Vz[k][j][i + 1] - (1. / r[i]) * Vz[k][j][i]);
      tau_yx[i] = tau_xy[i];
      tau_zx[i] = tau_xz[i];
      ...
      divV = DIM_EXPAND(0.5 * (Vx[k][j][i + 1] - Vx[k][j][i - 1]) * inv_dx1 +
                            Vx[k][j][i] * r_1,
                        +0.5 * (Vy[k][j + 1][i] - Vy[k][j - 1][i]) * inv_dx2,
                        +0.0);

      tau_zz[i] =
          2.0 * nu1 * r_1 * Vx[k][j][i] + (nu2 - (2.0 / 3.0) * nu1) * divV;
      ...
      divr = 2.0 *
             (Vx[k][j][i + 1] * fabs(x1[i + 1]) - Vx[k][j][i] * fabs(x1[i])) /
      ...
      scrh = x1r[i] * (Vy[k][j][i + 1] / x1[i + 1] - Vy[k][j][i] / x1[i]) *
             inv_dx1;
      ...
      divV = DIM_EXPAND(2.0 * r_1 * vi[VX1] + dxVx,
                        +r_1 * dyVy + r_1 * tan_1 * vi[VX2], +r_1 * s_1 * dzVz);
      ...
      tau_yx[i] = tau_xy[i];
      tau_yy[i] =
          2.0 * nu1 * (r_1 * dyVy + r_1 * vi[VX1]) + (nu2 - c23 * nu1) * divV;
      tau_zx[i] = tau_xz[i];
      tau_zz[i] =
          2.0 * nu1 *
              (r_1 * s_1 * dzVz + r_1 * vi[VX1] + tan_1 * r_1 * vi[VX2]) +
          (nu2 - c23 * nu1) * divV;

      r_1 = 1.0 / x1[i];
      ViS[i][MX1] =
          -0.5 * (tau_yy[i - 1] + tau_yy[i] + tau_zz[i - 1] + tau_zz[i]) * r_1;
      ViS[i][MX2] = 0.0;
      ViS[i][MX3] = 0.0;
...
#if HAVE_ENERGY
      ViF[i][ENG] =
          vi[VX1] * tau_xx[i] + vi[VX2] * tau_yx[i] + vi[VX3] * tau_zx[i];
#endif
    }
    ...
    for (j = beg; j <= end; j++) {
      ...
      tau_xx[j] = tau_xy[j] = tau_xz[j] = tau_yx[j] = tau_yy[j] = tau_yz[j] =
          tau_zx[j] = tau_zy[j] = tau_zz[j] = 0.0;
      ...
      divV = DIM_EXPAND(
          2.0 * r_1 * vi[VX1] + dxVx,
          +(sin(th[j + 1]) * Vy[k][j + 1][i] - fabs(sin(th[j])) * Vy[k][j][i]) *
              r_1 * one_dmu[j],
          +r_1 * s_1 * dzVz);
      ...
      tau_zz[j] =
          2.0 * nu1 *
              (r_1 * s_1 * dzVz + r_1 * vi[VX1] + tan_1 * r_1 * vi[VX2]) +
          (nu2 - c23 * nu1) * divV;
#endif /*tau_phiphi = 2eta1(1/rs dphiVphi + 1/r Vr + 1/r cot Vthita) + (eta2   \
          -2/3 eta1)divV */
...
#if HAVE_ENERGY
      ViF[j][ENG] =
          vi[VX1] * tau_xy[j] + vi[VX2] * tau_yy[j] + vi[VX3] * tau_zy[j];
#endif
    }
...
#if HAVE_ENERGY
      ViF[k][ENG] =
          vi[VX1] * tau_xz[k] + vi[VX2] * tau_yz[k] + vi[VX3] * tau_zz[k];
#endif
    } /*loop*/
  } /*sweep*/
}
