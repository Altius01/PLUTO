...
/*!
 *  Compute viscous fluxes for the LES model and add
 *  them to the rhs according to the sign of
 *
 *  f = N - aux[FMIN]*D,
 *
 *  where N, D > 0.
...

{
  ...
  /* ----------------------------------------------
     0. Allocate static memory, initialize
        pointers.
     ---------------------------------------------- */
  ...
  /* ----------------------------------------------
     3. Compute tot averaged integrated vorticity
     ---------------------------------------------- */
  /*
    Om2_ave = 0.0;
    for (k = KBEG; k <= KEND; k++){
    for (i = IBEG; i <= IEND; i++){
     ...
     MPI_Allreduce (&Om2_ave, &Om2_ave_glob, 1, MPI_PLUTO_REAL, MPI_SUM,
    MPI_COMM_WORLD); MPI_Barrier(MPI_COMM_WORLD); #else Om2_ave_glob = Om2_ave;
    #endif

    Om2_ave_glob /= (real)(grid[IDIR].np_int_glob*grid[KDIR].np_int_glob);
  */
  /* ----------------------------------------------
     4. Compute "f" function
     ---------------------------------------------- */
