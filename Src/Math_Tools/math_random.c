 ...
 *********************************************************************** */
{
  ...
  *idum = IA1 * (*idum - k * IQ1) -
          k * IR1; /* Compute idum=(IA1*idum) % IM1 without */
  if (*idum < 0)
    *idum += IM1; /* overflows by Schrag's method.        */
  k = idum2 / IQ2;
  idum2 = IA2 * (idum2 - k * IQ2) -
  ...
  if (iy < 1)
    iy += IMM1;
  if ((temp = AM * iy) > RNMX)
    return RNMX; /* Because users don't expect
                    endpoint values.*/
  else
    return temp;
}
