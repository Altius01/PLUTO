 ...
 *********************************************************************** */
{
  ...
  */
  ...
  DIM_EXPAND(data->Vs[BX1s] = ARRAY_BOX(0, NX3_TOT - 1, 0, NX2_TOT - 1, -1,
                                        NX1_TOT - 1, double);
             , data->Vs[BX2s] = ARRAY_BOX(0, NX3_TOT - 1, -1, NX2_TOT - 1, 0,
                                          NX1_TOT - 1, double);
             , data->Vs[BX3s] = ARRAY_BOX(-1, NX3_TOT - 1, 0, NX2_TOT - 1, 0,
                                          NX1_TOT - 1, double);)
#if (PHYSICS == ResRMHD) && (DIVE_CONTROL == CONSTRAINED_TRANSPORT)
  DIM_EXPAND(data->Vs[EX1s] = ARRAY_BOX(0, NX3_TOT - 1, 0, NX2_TOT - 1, -1,
                                        NX1_TOT - 1, double);
             , data->Vs[EX2s] = ARRAY_BOX(0, NX3_TOT - 1, -1, NX2_TOT - 1, 0,
                                          NX1_TOT - 1, double);
             , data->Vs[EX3s] = ARRAY_BOX(-1, NX3_TOT - 1, 0, NX2_TOT - 1, 0,
                                          NX1_TOT - 1, double);)
#endif
  ...
  DIM_EXPAND(data->Vps[BX1s] = ARRAY_BOX(0, NX3_TOT - 1, 0, NX2_TOT - 1, -1,
                                         NX1_TOT - 1, double);
             , data->Vps[BX2s] = ARRAY_BOX(0, NX3_TOT - 1, -1, NX2_TOT - 1, 0,
                                           NX1_TOT - 1, double);
             , data->Vps[BX3s] = ARRAY_BOX(-1, NX3_TOT - 1, 0, NX2_TOT - 1, 0,
                                           NX1_TOT - 1, double);)

#if (PHYSICS == ResRMHD) && (DIVE_CONTROL == CONSTRAINED_TRANSPORT)
  DIM_EXPAND(data->Vps[EX1s] = ARRAY_BOX(0, NX3_TOT - 1, 0, NX2_TOT - 1, -1,
                                         NX1_TOT - 1, double);
             , data->Vps[EX2s] = ARRAY_BOX(0, NX3_TOT - 1, -1, NX2_TOT - 1, 0,
                                           NX1_TOT - 1, double);
             , data->Vps[EX3s] = ARRAY_BOX(-1, NX3_TOT - 1, 0, NX2_TOT - 1, 0,
                                           NX1_TOT - 1, double);)
#endif
#endif /* HIGH_ORDER */
...
#if RESISTIVITY || HALL_MHD || MHD_LES || (PHYSICS == ResRMHD)
...
#if (PARTICLES != NO)
  if (cmd_line->prestart == NO)
    Particles_Set(data, grid);
#endif
  ...
  DIM_EXPAND(print("> Proc size:            %d", grid->np_int[IDIR]);
             , print(" X %d", grid->np_int[JDIR]);
             , print(" X %d", grid->np_int[KDIR]);)
  print("\n");
#ifdef PARALLEL
  print("> Parallel Directions: ");
  DIM_EXPAND(if (pardim[IDIR]) print(" X1");, if (pardim[JDIR]) print("/X2");
 ...
 *********************************************************************** */
{
  ...
  DIM_EXPAND(npx = cmd_line->nproc[IDIR];, npy = cmd_line->nproc[JDIR];
