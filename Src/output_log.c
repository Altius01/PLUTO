 ...
 *********************************************************************** */
{
  ...
  print("> nstep = %d; t = %0e; dt = %0e; %6.1f %%\n", g_stepNumber, g_time,
        g_dt, 100.0 * g_time / runtime->tstop);
#else
...
// Dts.invDt_hyp = xglob;
  ...
  print("  Max(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e % 0e % 0e\n",
        vc_max[RHO], vc_max[VX1], vc_max[VX2], vc_max[VX3], vc_max[BX1],
        vc_max[BX2], vc_max[BX3], vc_max[PRS], vc_max[PSI_GLM]);
  print("  Min(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e % 0e % 0e\n",
        vc_min[RHO], vc_min[VX1], vc_min[VX2], vc_min[VX3], vc_min[BX1],
        vc_min[BX2], vc_min[BX3], vc_min[PRS], vc_min[PSI_GLM]);
#else
  print("  Max(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e % 0e\n",
        vc_max[RHO], vc_max[VX1], vc_max[VX2], vc_max[VX3], vc_max[BX1],
        vc_max[BX2], vc_max[BX3], vc_max[PRS]);
  print("  Min(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e % 0e\n",
        vc_min[RHO], vc_min[VX1], vc_min[VX2], vc_min[VX3], vc_min[BX1],
        vc_min[BX2], vc_min[BX3], vc_min[PRS]);
#endif
#else
  print("  Max(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e\n", vc_max[RHO],
        vc_max[VX1], vc_max[VX2], vc_max[VX3], vc_max[BX1], vc_max[BX2],
        vc_max[BX3]);
  print("  Min(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e\n", vc_min[RHO],
        vc_min[VX1], vc_min[VX2], vc_min[VX3], vc_min[BX1], vc_min[BX2],
        vc_min[BX3]);
#endif
#endif
...
#if PHYSICS == ResRMHD
#ifdef GLM_MHD
  print("  Max(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e % 0e % 0e % 0e "
        "% 0e % 0e % 0e\n",
        vc_max[RHO], vc_max[VX1], vc_max[VX2], vc_max[VX3], vc_max[BX1],
        vc_max[BX2], vc_max[BX3], vc_min[EX1], vc_min[EX2], vc_min[EX3],
        vc_max[PRS], vc_min[CRG], vc_max[PSI_GLM]);
  print("  Min(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e % 0e % 0e % 0e "
        "% 0e % 0e % 0e\n",
        vc_min[RHO], vc_min[VX1], vc_min[VX2], vc_min[VX3], vc_min[BX1],
        vc_min[BX2], vc_min[BX3], vc_min[EX1], vc_min[EX2], vc_min[EX3],
        vc_min[PRS], vc_min[CRG], vc_min[PSI_GLM]);
#else
  print("  Max(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e % 0e % 0e  % "
        "0e % 0e\n",
        vc_max[RHO], vc_max[VX1], vc_max[VX2], vc_max[VX3], vc_min[EX1],
        vc_min[EX2], vc_min[EX3], vc_max[BX1], vc_max[BX2], vc_max[BX3],
        vc_max[PRS]);
  print("  Min(vc)   = % 0e  % 0e  % 0e  % 0e  % 0e  % 0e  % 0e % 0e % 0e  % "
        "0e % 0e\n",
        vc_min[RHO], vc_min[VX1], vc_min[VX2], vc_min[VX3], vc_min[EX1],
        vc_min[EX2], vc_min[EX3], vc_min[BX1], vc_min[BX2], vc_min[BX3],
        vc_min[PRS]);
#endif
#endif

#if PHYSICS == HD || PHYSICS == RHD
  print("  Max(vc)   = % 0e  % 0e  % 0e  % 0e % 0e\n", vc_max[RHO], vc_max[VX1],
        vc_max[VX2], vc_max[VX3], vc_max[PRS]);
  print("  Min(vc)   = % 0e  % 0e  % 0e  % 0e % 0e\n", vc_min[RHO], vc_min[VX1],
        vc_min[VX2], vc_min[VX3], vc_min[PRS]);
#endif
...
#endif
#if LES
  print(", Cs = %lf, Ys = %lf", g_maxCs, g_maxYs);
#endif
#if MHD_LES
  print(", Ds = %lf", g_maxDs);
#endif
 ...
 *********************************************************************** */
{
  ...
  sEmin =
      sEmin_glob /
      (np_glob + 1.e-6); /* Avoid division by zero when there're no particles */
 ...
 *********************************************************************** */
{
  ...
  if (g_stepNumber < 10)
    sprintf(str, "%7s", " ");
  else if (g_stepNumber < 100)
    sprintf(str, "%8s", " ");
  else if (g_stepNumber < 1000)
    sprintf(str, "%9s", " ");
  else if (g_stepNumber < 10000)
    sprintf(str, "%10s", " ");
  else if (g_stepNumber < 100000)
    sprintf(str, "%11s", " ");
  else
    sprintf(str, "%12s", " ");
