...
#if RADIATION
double g_absorptionCoeff =
    0.0; // Absorption coefficient (code_length^2/code_energy)
double g_scatteringCoeff =
    0.0; // Scattering coefficient (code_length^2/code_energy)
double g_radiationConst =
    1.0; // Radiation constant (4*StefanBoltzmannConstant/c)
         // (code_energy/(code_length^3*code_temperature^4))
double g_idealGasConst = 1.0; //(code_temperature) g_idealGasConst*prs/rho
...
#ifdef GLM_MHD
int glm_is_defined =
    1; /**< Integer used to tell Chombo to compute glm_ch
...
int d_condition =
    1; /**< Enable/disable printLoging when a certain cond. is verified */
#endif
