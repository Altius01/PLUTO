...
class DefineProblem(object):
    def __init__(self, work_dir, pluto_dir, auto_update):
        ...
        **Output**:
        ...
        if self.default[self.entries.index("PHYSICS")] == "ResRMHD":
            ...
            self.ReadOrBrowse(
                Ents=self.entries_ResRMHD,
                Defs=self.default_ResRMHD,
                Opts=self.options_ResRMHD,
                MenuTitle="ResRMHD Menu",
            )
    ...
    def AfterFlagLists(self):
        ...
        if self.flag_dict["FULL"]:
            self.options[self.entries.index("RECONSTRUCTION")] = [
                "FLAT",
                "LINEAR",
                "LimO3",
                "WENO3",
        ...
        if self.flag_dict["WITH-CHOMBO"]:
            self.options[self.entries.index("GEOMETRY")] = [
                "CARTESIAN",
                "CYLINDRICAL",
                "POLAR",
                "SPHERICAL",
            ...
            self.options[self.entries.index("TIME_STEPPING")] = [
                "EULER",
                "HANCOCK",
        ...
        if self.flag_dict["WITH-SB"]:
            ...
            self.options[self.entries.index("DIMENSIONS")] = ["2", "3"]
            self.default[self.entries.index("DIMENSIONS")] = "2"
            self.options[self.entries.index("BODY_FORCE")] = [
                "VECTOR",
                "POTENTIAL",
                "(VECTOR+POTENTIAL)",
            ]
    ...
    def ProcessHDModule(self):
        ...
        self.default_HD = ["NO", "IDEAL", "NO", "NO", "NO", "NO", "NO", "NO", "NO"]
        self.options_HD = [
            ["NO", "YES"],
            ["IDEAL", "PVTE_LAW", "ISOTHERMAL"],
    ...
    def AppendPlutoPathAndFlags(self):
        ...
        for de in dis_eff:
            if (de.upper() in self.mod_entries) and (
                self.mod_default[self.mod_entries.index(de.upper())] != "NO"
            ):
                self.pluto_path.append(de + "/")

        if ("INCLUDE_LES" in self.mod_entries) and (
            self.mod_default[self.mod_entries.index("INCLUDE_LES")] != "NO"
        ):
            self.pluto_path.append("LES/")

        if ("LES" in self.mod_entries) and (
            self.mod_default[self.mod_entries.index("LES")] != "NO"
        ):
            self.pluto_path.append("LES/")
        ...
        if (
            ...
            if (
                self.phymodule == "MHD"
                and self.mod_default[self.mod_entries.index("AMBIPOLAR_DIFFUSION")]
                != "NO"
            ):
            ...
            if (
                self.phymodule == "MHD"
                and self.mod_default[self.mod_entries.index("HALL_MHD")] != "NO"
            ):
                self.pluto_path.append("MHD/Hall_MHD/")

            if (
                self.phymodule == "MHD"
                and self.mod_default[self.mod_entries.index("MHD_LES")] != "NO"
            ):
                self.pluto_path.append("MHD/LES/")
        ...
        if (
            particles == "PARTICLES_LP"
            or particles == "PARTICLES_CR"
    ...
    def WriteKromeOpts(self):
        self.kromeoptstr = (
            ...
            + self.krome_dir
            + "networks/"
            + self.default_KROME[self.entries_KROME.index("NETWORK_FILE")]
        )
        ...
        if self.default_KROME[self.entries_KROME.index("COOLING_TYPE")] != "NONE":
            self.kromeoptstr += (
                " -cooling="
                + self.default_KROME[self.entries_KROME.index("COOLING_TYPE")]
            )

        if self.default_KROME[self.entries_KROME.index("HEATING_TYPE")] != "NONE":
            self.kromeoptstr += (
                " -heating="
                + self.default_KROME[self.entries_KROME.index("HEATING_TYPE")]
            )

        if self.default_KROME[self.entries_KROME.index("GAMMA_TYPE")] != "DEFAULT":
            self.kromeoptstr += (
                " -gamma=" + self.default_KROME[self.entries_KROME.index("GAMMA_TYPE")]
            )
        ...
        try:
            ...
            for x in self.entries_KROME:
                self.def_file_list.append(
                    ...
                    + x.ljust(28)
                    + "   "
                    + self.default_KROME[self.entries_KROME.index(x)]
                    + "\n"
                )
